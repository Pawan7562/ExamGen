  ...
  const handleSubmit = async (status = 'draft') => {
    ...
    try {
      ...
      const validQuestions = cleanedQuestions.filter(q => 
        q.questionText && q.questionText.length >= 5
      );
      ...
      const submitData = {
        ...
        scheduling: {
          startDate: formatToISO(examData.scheduling.startDate),
          endDate: formatToISO(examData.scheduling.endDate),
          timeZone: examData.scheduling.timeZone || 'UTC'
        },
      ...
      if (!submitData.course || submitData.course.length < 2) {
        toast.error('Course must be at least 2 characters long');
        setLoading(false);
        return;
      }
      
      if (!submitData.courseCode || submitData.courseCode.length < 2) {
        toast.error('Course code must be at least 2 characters long');
        setLoading(false);
        return;
      }
      
      if (!submitData.scheduling.startDate || !submitData.scheduling.endDate) {
