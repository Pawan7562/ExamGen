...
const ExamList = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (response.status === 400) {
          if (data.message && data.message.includes('Cannot change status from')) {
            toast.error(`❌ ${data.message}`);
            console.log('⚠️ Status transition not allowed:', data.message);
          } else if (data.message && data.message.includes('Cannot publish exam')) {
            toast.error(`❌ ${data.message}`);
            console.log('⚠️ Publishing validation failed:', data.message);
          } else {
          ...
          console.log('🔒 Authorization failed for exam:', examId);
        } else if (response.status === 404) {
          toast.error('❌ Exam not found');
          console.log('🔍 Exam not found:', examId);
        } else {
          toast.error(`❌ ${data.message || `Error updating exam status (${response.status})`}`);
        }
  ...
  const getStatusTransitionHelp = (currentStatus) => {
    const allowed = getAllowedStatusTransitions(currentStatus);
    if (allowed.length === 0) {
      return 'This exam is in a final state and cannot be changed.';
    }
    return `Can be changed to: ${allowed.join(', ')}`;
  };
