...
const createExam = async (req, res) => {
  try {
    ...
    // Send automatic email notifications to all eligible students
    try {
      console.log('📧 Sending automatic email notifications to students for new exam...');
      
      // Get all eligible students for this exam
      const eligibleStudents = await User.find({
        userType: 'student',
        isActive: true,
        $or: [
          { course: examData.course },
          { course: { $in: [examData.course] } },
          { courseCode: examData.courseCode }
        ]
      }).select('fullName email phoneNumber studentId course courseCode batch').exec();
      
      console.log(`📊 Found ${eligibleStudents.length} eligible students for exam notifications`);
      ...
      if (eligibleStudents.length > 0) {
        ...
        const emailPromises = eligibleStudents.map(async (student) => {
          try {
                ...
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                  ...
                  <div style="background: #f8fafc; padding: 30px; border-radius: 10px;">
                    <div style="background: white; padding: 25px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                      <h3 style="color: #2d3748; margin-top: 0;">📋 Exam Details</h3>
                      
                      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 20px 0;">
                        <div>
                          <p style="margin: 5px 0;"><strong>📚 Course:</strong> ${examData.course}</p>
                          <p style="margin: 5px 0;"><strong>📝 Course Code:</strong> ${examData.courseCode}</p>
                          <p style="margin: 5px 0;"><strong>⏱️ Duration:</strong> ${examData.settings.duration} minutes</p>
                        </div>
                        <div>
                          <p style="margin: 5px 0;"><strong>📅 Start Date:</strong> ${new Date(examData.scheduling.startDate).toLocaleString()}</p>
                          <p style="margin: 5px 0;"><strong>🏁 End Date:</strong> ${new Date(examData.scheduling.endDate).toLocaleString()}</p>
                          <p style="margin: 5px 0;"><strong>🎯 Total Points:</strong> ${examData.settings.totalPoints}</p>
                        </div>
                      </div>
                      
                      <div style="background: #e3f2fd; padding: 20px; border-radius: 8px; margin: 20px 0;">
                        <h4 style="color: #1a202c; margin-top: 0;">🎯 How to Access Your Exam</h4>
                        <ol style="color: #424242; line-height: 1.6;">
                          ...
                          <li>Find this exam and click "Start Exam"</li>
                          <li>Complete the exam within the specified time limit</li>
                        </ol>
                      </div>
                      
                      <div style="background: #fef3c7; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center;">
                        <p style="color: #2d3748; margin: 0; font-size: 16px;">💡 <strong>Pro Tip:</strong> Prepare well by reviewing the course material and managing your time effectively!</p>
                      </div>
                    </div>
                  </div>
                  
                  <div style="background: #2d3748; color: white; padding: 20px; text-align: center; border-radius: 0 0 10px 10px;">
                    <p style="margin: 0; font-size: 14px;">This is an automated notification from the Exam Monitor System.</p>
            ...
            console.log(`📧 Email sent successfully to ${student.email}:`, result.messageId);
            return { success: true, studentId: student.studentId, email: student.email, messageId: result.messageId };
          } catch (emailError) {
            console.error(`❌ Failed to send email to ${student.email}:`, emailError.message);
            return { success: false, studentId: student.studentId, email: student.email, error: emailError.message };
          }
        });
        
        // Also send SMS notifications to students with phone numbers
        const smsPromises = eligibleStudents.map(async (student) => {
          if (student.phoneNumber) {
            try {
              const result = await notificationService.sendSMS(
                student.phoneNumber,
                `🚀 New Exam Available: ${examData.title}. Check your email for details. Exam starts: ${new Date(examData.scheduling.startDate).toLocaleString()}`
              );
              
              console.log(`📱 SMS sent successfully to ${student.phoneNumber}:`, result.messageId);
              return { success: true, studentId: student.studentId, phone: student.phoneNumber, messageId: result.messageId };
            } catch (smsError) {
              console.error(`❌ Failed to send SMS to ${student.phoneNumber}:`, smsError.message);
              return { success: false, studentId: student.studentId, phone: student.phoneNumber, error: smsError.message };
            }
          }
          return { success: true, studentId: student.studentId, note: 'No phone number provided' };
        });
        ...
        const successfulSMS = smsResults.filter(r => r.status === 'fulfilled' && r.value.success).length;
        const totalStudents = eligibleStudents.length;
        
        console.log(`📊 Notification Summary: ${successfulEmails}/${totalStudents} emails, ${successfulSMS}/${totalStudents} SMS sent successfully`);
        
        // Update the exam document with notification tracking
        await Exam.findByIdAndUpdate(exam._id, {
          notifications: {
            ...
            successfulEmails,
            successfulSMS,
            failedEmails: totalStudents - successfulEmails,
            failedSMS: totalStudents - successfulSMS
          }
        });
        
      } else {
        console.log('⚠️ No eligible students found for exam notifications');
      }
    } catch (notificationError) {
      console.error('💥 Error sending automatic notifications:', notificationError);
      // Don't fail the exam creation if notifications fail
    }
    
    res.status(201).json({
      success: true,
      message: 'Exam created successfully and notifications sent to students',
      data: exam
    });
  } catch (error) {
